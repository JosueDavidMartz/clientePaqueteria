...
public class ClienteDAO {
    public static List<Cliente> obtenerClientes() {
        ...
        try {
            ...
            if (respuesta != null && respuesta.getCodigoRespuesta() == HttpURLConnection.HTTP_OK) {
                Gson gson = new Gson();
                Type tipoLista = new TypeToken<List<Cliente>>(){}.getType();
                clientes = gson.fromJson(respuesta.getContenido(), tipoLista);
            } else {
                // Manejar respuestas que no son HTTP_OK o respuesta nula
                System.err.println("Error en la respuesta: " + (respuesta != null ? respuesta.getCodigoRespuesta() : "Respuesta nula"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
